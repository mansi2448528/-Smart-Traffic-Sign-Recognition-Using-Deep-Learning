...
def classify(image):
    """Predicts the class of the uploaded traffic sign image."""
    img = image.resize((30, 30))
    img = np.expand_dims(img, axis=0)
    img = np.array(img)
    pred = np.argmax(model.predict(img))
    return classes.get(pred, "Unknown Sign")

# Streamlit UI
st.set_page_config(page_title="Traffic Sign Recognition", layout="centered")
st.title("🚦 Traffic Sign Recognition System")
st.markdown("Upload a traffic sign image to classify it using our trained CNN model.")
